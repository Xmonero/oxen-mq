...
TEST_CASE("plain-text connections", "[plaintext][connect]") {
    std::string listen = "tcp://127.0.0.1:4455";
    QueneroMQ server{get_logger("S» "), LogLevel::trace};
...
TEST_CASE("unique connection IDs", "[connect][id]") {
    ...
    QueneroMQ client1{get_logger("C1» "), LogLevel::trace};
    QueneroMQ client2{get_logger("C2» "), LogLevel::trace};
    client1.start();
    client2.start();
...
TEST_CASE("SN disconnections", "[connect][disconnect]") {
    std::vector<std::unique_ptr<QueneroMQ>> lmq;
    ...
    for (int i = 0; i < pubkey.size(); i++) {
        lmq.push_back(std::make_unique<QueneroMQ>(
...
TEST_CASE("SN single worker test", "[connect][worker]") {
    ...
    QueneroMQ client{get_logger("B» "), LogLevel::trace};
    client.start();
