...
void QueneroMQ::rebuild_pollitems() {
    pollitems.clear();
...
void QueneroMQ::setup_external_socket(zmq::socket_t& socket) {
...
void QueneroMQ::setup_outgoing_socket(zmq::socket_t& socket, string_view remote_pubkey) {
...
ConnectionID QueneroMQ::connect_sn(string_view pubkey, std::chrono::milliseconds keep_alive, string_view hint) {
...
ConnectionID QueneroMQ::connect_remote(string_view remote, ConnectSuccess on_connect, ConnectFailure on_failure,
...
void QueneroMQ::disconnect(ConnectionID id, std::chrono::milliseconds linger) {
...
QueneroMQ::proxy_connect_sn(string_view remote, string_view connect_hint, bool optional, bool incoming_only, bool outgoing_only, std::chrono::milliseconds keep_alive) {
...
std::pair<zmq::socket_t *, std::string> QueneroMQ::proxy_connect_sn(bt_dict_consumer data) {
...
void QueneroMQ::proxy_close_connection(size_t index, std::chrono::milliseconds linger) {
...
void QueneroMQ::proxy_expire_idle_peers() {
...
void QueneroMQ::proxy_conn_cleanup() {
...
void QueneroMQ::proxy_connect_remote(bt_dict_consumer data) {
...
void QueneroMQ::proxy_disconnect(bt_dict_consumer data) {
...
void QueneroMQ::proxy_disconnect(ConnectionID conn, std::chrono::milliseconds linger) {
