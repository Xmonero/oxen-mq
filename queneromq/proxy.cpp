...
void QueneroMQ::proxy_send(bt_dict_consumer data) {
...
void QueneroMQ::proxy_reply(bt_dict_consumer data) {
    bool have_conn_id = false;
...
void QueneroMQ::proxy_control_message(std::vector<zmq::message_t>& parts) {
    // We throw an uncaught exception here because we only generate control messages internally in
    // queneromq code: if one of these condition fail it's a queneromq bug.
    if (parts.size() < 2)
        throw std::logic_error("QueneroMQ bug: Expected 2-3 message parts for a proxy control message");
    ...
    throw std::runtime_error("QueneroMQ bug: Proxy received invalid control command: " +
            std::string{cmd} + " (" + std::to_string(parts.size()) + ")");
}

void QueneroMQ::proxy_loop() {
    ...
    for (size_t i = 0; i < bind.size(); i++) {
        ...
        LMQ_LOG(info, "QueneroMQ listening on ", bind[i].first);
...
// reason)
bool QueneroMQ::proxy_handle_builtin(size_t conn_index, std::vector<zmq::message_t>& parts) {
    ...
    if (cmd == "REPLY") {
        ...
        if (parts.size() == (1 + incoming) && cmd == "UNKNOWNCOMMAND") {
            ...
            LMQ_LOG(debug, "Received plain UNKNOWNCOMMAND; remote is probably an older queneromq. Ignoring.");
            return true;
        }
...
void QueneroMQ::proxy_process_queue() {
